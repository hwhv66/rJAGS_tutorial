...
Data for this example comes from Peter Hoff's *A First Course in Bayesian Statistical Methods*. We will use the `sparrows.csv` data set.

```{r}
sparrows <- read.csv("sparrows.csv")
sparrows <- sparrows[ , c("fledged", "age")]
```

#### Background Information

*The following passage is sourced directly from the book mentioned above with little editing.*

A sample from a population of 52 female song sparrows was studied over the course of a summer and their reproductive activities were recorded. In particular, the age and number of new offspring were recorded for each sparrow (Arcese et al, 1992).

```{r}

boxplot(fledged ~ age, data = sparrows, xlab = "age", ylab = "offspring")

```

The figure above shows boxplots of the number of offspring versus age. The figure indicates that two-year-old birds in this population had the highest median reproductive success, with the number of offspring declining beyond two years of age. This is not surprising from a biological point of view: One-year-old birds are in their first mating season and are relatively inexperienced compared to two-year-old birds. As birds age beyond two years they experience a general decline in health and activity.

Suppose we wish to fit a probability model to these data, perhaps to understand the relationship between age and reproductive success, or to make population forecasts for this group of birds. Since the number of offspring for each bird is a non-negative integer $\{0,1,2,...\}$, a simple probability model for the response $Y$ (`fledged`) conditional on the predictor $X$ (`age`) would be a Poisson model. 

One possibility would be to estimate $X\beta$ separately for each age group. However, the number of birds of each age is small and so the estimates of $X\beta$ would be imprecise. To add stability to the estimation we will assume that the mean number of offspring is a smooth function of age. We will want to allow this function to be quadratic so that we can represent the increase in mean offspring while birds mature and the decline they experience thereafter. We will use a log link function to account for the fact that $Y$ must be positive.

$$
X_i = [1, x_i, x_i^2]^T\\
\log(E[Y]) = X\beta \\
Y | \beta, X \sim \mathcal{Pois}(\exp\{ X\beta \})
$$
Our poisson regression model will be in the form
$$
\log(E[Y]) = \beta_0 + x_i \beta_1 + x_i^2 \beta_2
$$

```{r}

# modify the data set
sparrows[ , "intcpt"] <- rep(1, nrow(sparrows))
...
mod_string_pois <- "model {
  for (i in 1:length(y)) {
    y[i] ~ dpois(lmbda[i])
    
    # log link function
    log(lmbda[i]) <- b[1] * intcpt[i] + b[2] * age[i] + b[3] * age2[i]
  }
...
inits_pois <- function() {
  inits <- list("b" = rnorm(3, 0.0, 1.0 / 25.0))
}

data_pois <- list(y = sparrows$fledged,
                  intcpt = sparrows$intcpt,
                  age = sparrows$age,
                  age2 = sparrows$age2)
...
# compile model
mod_pois <- jags.model(
  file = textConnection(mod_string_pois),
  data = data_pois,
  n.chains = 3
  )
...
# burn-in
update(mod_pois, n.iter = 1000)

# sampling
mod_sim_pois <- coda.samples(model = mod_pois,
                             variable.names = params_pois,
                             n.iter = 1e4)

comb_chains_pois <- as.mcmc(do.call(rbind, mod_sim_pois))
...
# bad autocorrelation
autocorr.diag(mod_sim_pois)

```

```{r}

effectiveSize(mod_sim_pois)

```

We can try to take more samples and use some thinning to reduce the autocorrelation in order to improve the effective sample size.
...
Sampling this many samples and thinning means our model will take significantly longer to fit.
...
# sampling
mod_sim_pois2 <- coda.samples(model = mod_pois,
                             variable.names = params_pois,
                             thin = 10,
                             n.iter = 1e5)

comb_chains_pois2 <- as.mcmc(do.call(rbind, mod_sim_pois))
...
# slightly better on their surface
plot(mod_sim_pois2, density = FALSE)

```

```{r}

autocorr.diag(mod_sim_pois2)

```

```{r}

# better effective sample size by a factor of ~10...
effectiveSize(mod_sim_pois2)

```

```{r}

pois_sum <- summary(mod_sim_pois2)
pois_sum
...
X <- data.matrix(cbind(sparrows$intcpt, sparrows$age, sparrows$age2))
pois_coefs <- c(pois_sum$statistics[1, 1], 
                pois_sum$statistics[2, 1],
                pois_sum$statistics[3, 1])

# must exponentiate to reverse effect of log link fcn
yhat <- exp(X %*% pois_coefs)
resids_pois <- yhat - sparrows$fledged

```

```{r}

plot(sparrows$fledged, resids_pois)
abline(h = mean(resids_pois), col = "red")
...
What if we want to know the probability that one bird will have more offspring than another?

We must first generate *posterior predictive distributions* for each bird.

```{r}

# randomly pick two birds from data set
### samples with my seed: ###
# 1 5 25
# 1 3 9
sample1 <- X[sample(c(1:52), size = 1), ]
sample2 <- X[sample(c(1:52), size = 1), ]

# generate MC estimate for mean of birds matching the characteristics of samples
loglam1 <- comb_chains_pois2 %*% sample1
loglam2 <- comb_chains_pois2 %*% sample2
...
The following two plots show the posterior predictive dsitributions for samples `1` and `2`. The red line indicates the mean while the blue lines indicate the $95%$ HPD interval.

```{r}

# density estimate of lam1 with mean and 95% HPD Interval
plot(density(lam1))
abline(v = mean(lam1), col = "red")
abline(v = HPDinterval(as.mcmc(lam1))[1, "lower"], col = "blue")
abline(v = HPDinterval(as.mcmc(lam1))[1, "upper"], col = "blue")
...
# density estimate of lam2
plot(density(lam2))
abline(v = mean(lam2), col = "red")
abline(v = HPDinterval(as.mcmc(lam2))[1, "lower"], col = "blue")
abline(v = HPDinterval(as.mcmc(lam2))[1, "upper"], col = "blue")


```

The question of probability that one bird will have more offspring than another can be answered by taking Monte Carlo samples of poisson distributions for `lam1` and `lam2`.

The following code displays the posterior predicitive distribution (poisson) of both birds for comparison.

```{r}

nsim <- nrow(comb_chains_pois2)
y1 <- rpois(nsim, lam1)
y2 <- rpois(nsim, lam2)

plot(table(factor(y1)) / nsim, col = "blue",
     xlab = "Offspring", 
     ylab = "Probability Mass")
points(table(factor(y2 + 0.2)) / nsim, col = "red")

```

To estimate the probability that the older bird has more offspring than the younger bird in a given year we can simple find the proportion of samples in which `y1` is greeater than `y2`:
...
Essentially all of the posterior probability density is above $0$, suggesting that this coefficient is positive (and consequently that bad health is associated with more visits). We obtained this result using a relatively noninformative prior. What if we use a prior that strongly favors values near $0$? Let’s repeat the analysis with a normal prior on the badh coefficient that has mean $0$ and standard deviation $0.2$, so that the prior probability that the coefficient is less than $0.6$ is > $0.998$. We will also use a small variance on the prior for the interaction term involving badh (standard deviation $0.01$ because this coefficient is on a much smaller scale).

```{r}

curve(dnorm(x, mean=0.0, sd=sqrt(1e4)),
      from = -3.0, to = 3.0, ylim = c(0.0, 3.0), lty = 2,
      main = "b_badh", ylab = "density", xlab = "b_badh")
curve(dnorm(x, mean = 0.0, sd = 0.2),
      from = -3.0, to = 3.0, col = "red", lty = 2, add = TRUE)
lines(density(mod_csim[,"b_badh"]))
lines(density(mod2_csim[,"b_badh"]), col = "red")
legend("topleft",
       legend = c("noninformative prior",
                "posterior",
                "skeptical prior",
                "posterior"),
       lty = c(2,1,2,1),
...
legend("topleft", legend = c("noninformative prior",
                             "posterior",
                             "skeptical prior",
                             "posterior"),
       lty = c(2,1,2,1),
